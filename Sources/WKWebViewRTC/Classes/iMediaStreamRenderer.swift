...
public class iMediaStreamRenderer : NSObject, RTCVideoViewDelegate {
	...
	init(
		...
		NSLog("iMediaStreamRenderer#init()")
	...
	deinit {
		NSLog("iMediaStreamRenderer#deinit()")
	}

	func run() {
		NSLog("iMediaStreamRenderer#run()")

		self.videoView.delegate = self
	}

	func render(_ pluginMediaStream: iMediaStream) {
		NSLog("iMediaStreamRenderer#render()")
	...
	func mediaStreamChanged() {
		NSLog("iMediaStreamRenderer#mediaStreamChanged()")
		...
		if oldRtcVideoTrack != nil && self.rtcVideoTrack != nil &&
			...
			NSLog("iMediaStreamRenderer#mediaStreamChanged() | same video track as before")
		}
	...
	func refresh(_ data: NSDictionary) {
		...
		NSLog("iMediaStreamRenderer#refresh() [elementLeft:%@, elementTop:%@, elementWidth:%@, elementHeight:%@, videoViewWidth:%@, videoViewHeight:%@, visible:%@, opacity:%@, zIndex:%@, mirrored:%@, clip:%@, borderRadius:%@]",
...
//		self.elementView.backgroundColor = color
	...
	func stop() {
		NSLog("iMediaStreamRenderer | video stop")
	...
	func close() {
		NSLog("iMediaStreamRenderer#close()")
		self.closed = true
		self.reset()
	...
	fileprivate func reset() {
		NSLog("iMediaStreamRenderer#reset()")
